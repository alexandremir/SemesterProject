...
def train(args, train_loader, disp_net, pose_net, optimizer, epoch_size, logger, train_writer):
    ...
    for i, (tgt_img, ref_imgs, intrinsics, intrinsics_inv) in enumerate(train_loader):
        ...
        poses, poses_inv = compute_pose_with_inv(pose_net, tgt_img, ref_imgs,intrinsics)
        #if poses is None:
...
def compute_pose_with_inv(pose_net, tgt_img, ref_imgs, intrinsics):
    new_poses_list_final = []
    new_poses_inv_list_final = []
    for i,ref_img in enumerate(ref_imgs):
        new_poses_list = []
        new_poses_inv_list = []
        poses = pose_net(tgt_img, ref_img)
        poses_inv = pose_net(ref_img, tgt_img)
        poses = poses.cpu().detach().numpy()
        poses_inv = poses_inv.cpu().detach().numpy()
        Rs = []
        Rsinv = []
        ts = []
        tsinv = []
        # ref_img_detached = ref_img.unsqueeze(0).cpu().detach().numpy()
        # tgt_img_detached = tgt_img.unsqueeze(0).cpu().detach().numpy()
        # ref_img_detached = ref_img.squeeze(0)
        # tgt_img_detached = tgt_img.squeeze(0)
        #print('np.shape(ref_img_detached) : ',np.shape(ref_img))
        #for i in range(2):
        K = intrinsics[i].cpu().numpy()
        R, t = pose(ref_img[i], tgt_img[i],K)
        Rinv, tinv = pose(tgt_img[i], ref_img[i],K)
        new_poses_list.append(np.concatenate((R,t*poses[i]), axis=None))
        new_poses_inv_list.append(np.concatenate((Rinv,tinv*poses_inv[i]),axis=None))

        new_poses_list = torch.FloatTensor(new_poses_list).cuda()
        new_poses_inv_list = torch.FloatTensor(new_poses_inv_list).cuda()

        new_poses_list_final.append(new_poses_list)
        new_poses_inv_list_final.append(new_poses_inv_list)

    return new_poses_list_final, new_poses_inv_list_final
#
...
#     print('poses size', np.shape(poses))
#     print('inv poses size', np.shape(poses_inv))
#     print('poses length : ',len(poses))
#
#     return poses, poses_inv


def pose(img1,img2, camera_matrix):

    img1 = np.uint8(img1.cpu().numpy().transpose(1, 2, 0))
    img2 = np.uint8(img2.cpu().numpy().transpose(1, 2, 0))
    ...
    if len(good_matches) < 6 :
        ...
        #     if m.distance < ratio_thresh * n.distance:
        #         good_matches.append(m)
        print('Good matches < 6')
        R = np.zeros((3,))
        t = np.zeros((3,1))
    else :
        img_matches = np.empty((max(img1.shape[0], img2.shape[0]), img1.shape[1]+img2.shape[1], 3), dtype=np.uint8)
        cv.drawMatches(img1, keypoints1, img2, keypoints2, good_matches, img_matches, flags=cv.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
        print(len(good_matches),' of good matches : ' )
        kp1_match = []
        kp2_match = []
        ...
        for mat in good_matches:
            ...
            kp1_match.append((x1, y1))
            kp2_match.append((x2, y2))

        E, mask_e = cv.findEssentialMat(np.float32(kp1_match), np.float32(kp2_match), focal=1.0, pp=(0., 0.),
                                       method=cv.RANSAC, prob=0.999, threshold=0.001)

        points, R, t, mask_RP, triangulatedPoints = cv.recoverPose(E, np.float32(kp1_match), np.float32(kp2_match), camera_matrix ,mask=mask_e, distanceThresh=50.0)
